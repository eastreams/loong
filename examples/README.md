...
- `plugins-process/native-extension-python/` -- checked-in governed Python process-stdio native extension example.
- `plugins-process/native-extension-javascript/` -- checked-in governed JavaScript process-stdio native extension example.
- `plugins-process/native-extension-typescript/` -- checked-in governed TypeScript process-stdio native extension example.
- `plugins-process/native-extension-go/` -- checked-in governed Go process-stdio native extension example.
- `plugins-process/native-extension-rust/` -- checked-in governed Rust process-stdio native extension example.
- `plugins-process/native-extension-trusted-host-javascript/` -- checked-in trusted-host JavaScript extension package with bounded `turn_start`/`turn_end` host-hook probes, per-hook and per-surface trusted specs, and shell-first surface declarations for the built-in `command_palette`, `settings_flow`, `startup_onboarding`, plus the custom `sidebar_widget` lane.
