...
- `plugins-process/native-extension-trusted-host-javascript/` -- checked-in trusted-host JavaScript extension package with bounded `turn_start`/`turn_end` host-hook probes plus shell-first TUI surface declarations for the built-in `command_palette`, `settings_flow`, `startup_onboarding`, and the custom `sidebar_widget` lane.
