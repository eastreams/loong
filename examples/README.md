...
- `plugins-process/native-extension-go/` -- checked-in governed Go process-stdio native extension example with per-method specs for `extension/event`, `extension/command`, and `extension/resource`.
- `plugins-process/native-extension-rust/` -- checked-in governed Rust process-stdio native extension example with per-method specs for `extension/event`, `extension/command`, and `extension/resource`.
- `plugins-process/channel-bridge-javascript/` -- checked-in managed bridge JavaScript reference package with channel bridge runtime contract metadata and per-operation specs for `send_message`, `receive_batch`, `ack_inbound`, and `complete_batch`.
- `plugins-process/native-extension-trusted-host-javascript/` -- checked-in trusted-host JavaScript extension package with bounded `turn_start`/`turn_end` host-hook probes, per-hook/per-action/per-surface trusted specs, and shell-first surface declarations for the built-in `command_palette`, `settings_flow`, `startup_onboarding`, plus the custom `sidebar_widget` lane.
