...
- `plugins-process/native-extension-trusted-host-javascript/` -- checked-in trusted-host JavaScript extension package with bounded `turn_start`/`turn_end` host-hook probes and shell-first TUI surface declarations for `command_palette`, `settings_flow`, and `startup_onboarding`.
