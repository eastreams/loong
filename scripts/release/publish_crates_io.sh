...
usage() {
  cat <<'EOF'
Usage: scripts/release/publish_crates_io.sh [--publish] [--from <package>] [--help]

Default behavior is a dry run:
  scripts/release/publish_crates_io.sh

Real publish mode:
  scripts/release/publish_crates_io.sh --publish

Resume from a package in the publish chain:
  scripts/release/publish_crates_io.sh --from loong-bridge-runtime
EOF
}
