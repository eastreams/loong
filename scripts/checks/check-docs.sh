...
RELEASE_DOCS_STRICT="${LOONG_RELEASE_DOCS_STRICT:-${LOONG_RELEASE_DOCS_STRICT:-}}"
...
if [ -n "${RELEASE_DOCS_STRICT:-}" ]; then
    case "${RELEASE_DOCS_STRICT}" in
        ...
        *)
            echo "FAIL: invalid LOONG_RELEASE_DOCS_STRICT/LOONG_RELEASE_DOCS_STRICT value '${RELEASE_DOCS_STRICT}' (expected 0/1)"
            exit 1
            ;;
    esac
...
if ! "$REPO_ROOT/scripts/checks/check_governance_docs_consistency.sh"; then
    ERRORS=$((ERRORS + 1))
fi
