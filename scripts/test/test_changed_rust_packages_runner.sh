...
SCRIPT_SOURCE="$REPO_ROOT/scripts/ci/test_changed_rust_packages.sh"
DAEMON_RUNNER_SOURCE="$REPO_ROOT/scripts/ci/run_selected_daemon_tests.sh"
...
EOF
  ...
  cat >"$fixture_root/scripts/ci/rust_changed_packages.py" <<EOF
#!/usr/bin/env python3
import json
import sys
...
if format_value == "json":
    ...
    for package in packages:
        print(package)
EOF
  chmod +x "$fixture_root/scripts/ci/rust_changed_packages.py"

  cat >"$fixture_root/scripts/ci/daemon_changed_test_targets.py" <<EOF
#!/usr/bin/env python3
print("""$daemon_targets""".strip())
EOF
  chmod +x "$fixture_root/scripts/ci/daemon_changed_test_targets.py"

  cat >"$fixture_root/scripts/dev/cargo-local-toolchain.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
EOF
  chmod +x "$fixture_root/scripts/dev/cargo-local-toolchain.sh"
...
run_explicit_paths_all_features_test() {
  ...
  (
    ...
    FAKE_INVOCATION_LOG="$invocation_log" \
      ...
      ./scripts/ci/test_changed_rust_packages.sh --all-features crates/daemon/src/gateway/openai_compat.rs >"$output_file" 2>&1
  )
...
run_explicit_paths_mixed_package_test() {
  ...
  (
    ...
    FAKE_INVOCATION_LOG="$invocation_log" \
      FAKE_BUILD_DIR="$build_dir" \
      ./scripts/ci/test_changed_rust_packages.sh crates/app/src/lib.rs >"$output_file" 2>&1
  )
