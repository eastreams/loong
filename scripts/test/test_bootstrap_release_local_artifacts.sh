...
SCRIPT_UNDER_TEST="$REPO_ROOT/scripts/release/bootstrap_release_local_artifacts.sh"
...
make_fixture_repo() {
  ...
  cp "$REPO_ROOT/scripts/checks/check-docs.sh" "$fixture/scripts/checks/check-docs.sh"
  cp "$REPO_ROOT/scripts/checks/check_governance_docs_consistency.sh" \
    "$fixture/scripts/checks/check_governance_docs_consistency.sh"
  cp "$REPO_ROOT/scripts/release/release_artifact_lib.sh" "$fixture/scripts/release/release_artifact_lib.sh"
  chmod +x "$fixture/scripts/checks/check-docs.sh"
  chmod +x "$fixture/scripts/checks/check_governance_docs_consistency.sh"
...
run_bootstrap_roundtrip_test() {
  ...
  if (
    cd "$fixture" &&
      LOONG_RELEASE_DOCS_STRICT=1 scripts/checks/check-docs.sh >"$strict_before" 2>&1
  ); then
  ...
  (
    cd "$fixture" &&
      LOONG_RELEASE_DOCS_STRICT=1 scripts/checks/check-docs.sh >"$strict_after" 2>&1
  )
...
run_prerelease_bootstrap_roundtrip_test() {
  ...
  (
    cd "$fixture" &&
      LOONG_RELEASE_DOCS_STRICT=1 scripts/checks/check-docs.sh >/dev/null
  )
}
...
run_release_doc_highlights_required_test() {
  ...
  if (
    cd "$fixture" &&
      LOONG_RELEASE_DOCS_STRICT=1 scripts/checks/check-docs.sh >"$strict_output" 2>&1
  ); then
