...
SCRIPT_SOURCE="$REPO_ROOT/scripts/ci/run_selected_daemon_tests.sh"
...
EOF
  ...
  (
    ...
    PATH="$stub_dir:$PATH" \
      ...
      ./scripts/ci/run_selected_daemon_tests.sh --all-features --include-lib-bins daemon_smoke daemon_feishu >"$output_file" 2>&1
  )
