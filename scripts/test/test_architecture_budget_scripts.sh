...
. "$REPO_ROOT/scripts/checks/architecture_budget_lib.sh"
...
make_fixture_repo() {
  ...
  cp "$REPO_ROOT/scripts/checks/architecture_budget_lib.sh" "$fixture/scripts/checks/architecture_budget_lib.sh"
  cp "$REPO_ROOT/scripts/checks/check_architecture_boundaries.sh" "$fixture/scripts/checks/check_architecture_boundaries.sh"
  chmod +x \
    "$fixture/scripts/checks/architecture_budget_lib.sh" \
    "$fixture/scripts/checks/check_architecture_boundaries.sh"
...
run_check_fails_on_missing_hotspot_test() {
  ...
  if (
    cd "$fixture" &&
      LOONG_ARCH_STRICT=true scripts/checks/check_architecture_boundaries.sh >"$output_file" 2>&1
  ); then
