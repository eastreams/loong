...
. "$REPO_ROOT/scripts/release/release_artifact_lib.sh"
...
assert_single_cli_bin_surface() {
  ...
  assert_not_contains "scripts/install/install.ps1" 'LegacyBinName'
  assert_not_contains "scripts/install/install.ps1" 'Installed compatible loong'
}

assert_release_docs_gates() {
  assert_contains ".github/workflows/release.yml" "scripts/release/bootstrap_release_local_artifacts.sh"
  assert_contains ".github/workflows/release.yml" "LOONG_RELEASE_DOCS_STRICT=1 scripts/checks/check-docs.sh"
