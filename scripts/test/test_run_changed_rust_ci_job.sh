...
SCRIPT_SOURCE="$REPO_ROOT/scripts/ci/run_changed_rust_ci_job.sh"
...
make_fixture_repo() {
  ...
  cat >"$fixture_root/scripts/ci/test_changed_rust_packages.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
printf 'changed %s\n' "$*" >>"$FAKE_INVOCATION_LOG"
EOF
...
run_pull_request_fast_lane_test() {
  ...
  (
    ...
    PATH="$fixture_root/stub:$PATH" \
      ...
      ./scripts/ci/run_changed_rust_ci_job.sh --all-features
  )
...
run_non_pr_full_workspace_test() {
  ...
  (
    ...
    PATH="$fixture_root/stub:$PATH" \
      ...
      ./scripts/ci/run_changed_rust_ci_job.sh
  )
