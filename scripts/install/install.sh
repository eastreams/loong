...
usage() {
  cat <<'USAGE'
Usage: ./scripts/install/install.sh [--prefix <dir>] [--onboard] [--version <tag>] [--source] [--target-libc <gnu|musl>]
...
if [[ -n "${script_dir}" ]]; then
  candidate_repo_root="$(cd "${script_dir}/../.." && pwd)"
  if [[ -f "${candidate_repo_root}/Cargo.toml" ]]; then
    repo_root="${candidate_repo_root}"
  fi
fi

if [[ -n "${repo_root}" && -f "${repo_root}/scripts/release/release_artifact_lib.sh" ]]; then
    ...
    if [[ -n "${rc_file}" ]]; then
      ...
      if [[ -n "${rc_file}" ]]; then
        if ! grep -qF "${path_line}" "${rc_file}" 2>/dev/null; then
          ...
          if [[ -s "${rc_file}" ]] && [[ "$(tail -c 1 "${rc_file}" 2>/dev/null | wc -l)" -eq 0 ]]; then
