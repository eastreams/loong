...
collect_daemon_test_targets() {
  "$PYTHON_BIN" scripts/ci/daemon_changed_test_targets.py --format names "$@"
}
...
while IFS= read -r line; do
  line="${line%$'\r'}"
  package_names+=("$line")
done < <("$PYTHON_BIN" scripts/ci/rust_changed_packages.py --format names "${changed_files[@]}")
...
if [[ "${#other_packages[@]}" -gt 0 ]]; then
  ...
  ./scripts/dev/cargo-local-toolchain.sh "${cargo_args[@]}"
fi
...
if [[ "$run_daemon" -eq 1 ]]; then
  ...
  ./scripts/ci/run_selected_daemon_tests.sh "${daemon_runner_args[@]}"
fi
