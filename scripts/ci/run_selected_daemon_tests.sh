...
if [[ "${#selected_targets[@]}" -eq 0 ]]; then
  echo "usage: scripts/ci/run_selected_daemon_tests.sh [--all-features] [--include-lib-bins] <target> [<target> ...]" >&2
  exit 1
fi
...
./scripts/dev/cargo-local-toolchain.sh "${build_args[@]}" >"$build_output_json"
