...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, Default)]
#[serde(default)]
pub struct PluginNativeExtensionEventSpec {
    pub event: String,
    pub label: Option<String>,
    pub summary: Option<String>,
    pub sample_payload_json: Option<String>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, Default)]
#[serde(default)]
pub struct PluginTrustedHostHookSpec {
    pub hook: String,
    pub label: Option<String>,
    pub summary: Option<String>,
    pub sample_payload_json: Option<String>,
...
impl PluginTranslator {
    ...
    pub fn translate_descriptor(&self, descriptor: &PluginDescriptor) -> PluginIR {
        ...
        let channel_bridge = derive_channel_bridge_contract(&descriptor.manifest);
...
fn plugin_manifest_contract_is_invalid(ir: &PluginIR) -> bool {
    let Some(channel_bridge) = ir.channel_bridge.as_ref() else {
        return false;
    };

    !channel_bridge.readiness.ready
}

fn format_invalid_manifest_contract_reason(ir: &PluginIR) -> String {
    let Some(channel_bridge) = ir.channel_bridge.as_ref() else {
        return "plugin manifest contract is invalid".to_owned();
    };

    let missing_fields = channel_bridge.readiness.missing_fields.join(", ");
...
fn derive_channel_bridge_contract(
...
pub fn plugin_native_extension_declarations_from_metadata(
    ...
    declarations.event_specs = normalized_event_specs_with_issue(
        metadata,
    ...
    declarations.host_hook_specs = normalized_host_hook_specs_with_issue(
        metadata,
...
fn validate_plugin_native_extension_declarations(
    ...
    for spec in &declarations.event_specs {
        if !declarations.events.iter().any(|event| event == &spec.event) {
            declarations.metadata_issues.push(format!(
    ...
    for spec in &declarations.host_hook_specs {
        if !TRUSTED_HOST_READ_ONLY_EXTENSION_HOOKS.contains(&spec.hook.as_str()) {
            declarations.metadata_issues.push(format!(
                "invalid trusted host hook spec `{}`; supported hooks are {}",
                spec.hook,
                TRUSTED_HOST_READ_ONLY_EXTENSION_HOOKS.join(", ")
            ));
            continue;
        ...
        if !declarations
            .host_hooks
            .iter()
            .any(|hook| hook == &spec.hook)
        {
            declarations.metadata_issues.push(format!(
                "trusted host hook spec `{}` requires a matching declaration in loong_extension_host_hooks_json",
                spec.hook
            ));
        }
    }
    ...
    declarations.host_hook_specs.sort_by_key(|spec| {
        declarations
            .host_hooks
            .iter()
    ...
    declarations.event_specs.sort_by_key(|spec| {
        declarations
            .events
            .iter()
    ...
    declarations.tui_surface_specs.sort_by_key(|spec| {
        declarations
            .tui_surfaces
            .iter()
            .position(|surface| surface == &spec.surface)
            .unwrap_or(usize::MAX)
    });
}
...
fn normalized_host_hook_specs_with_issue(
    metadata: &BTreeMap<String, String>,
    key: &str,
    metadata_issues: &mut Vec<String>,
) -> Vec<PluginTrustedHostHookSpec> {
    let Some(raw_value) = metadata.get(key) else {
        return Vec::new();
    };
    if raw_value.trim().is_empty() {
    ...
    let parsed_specs = match serde_json::from_str::<BTreeMap<String, serde_json::Value>>(raw_value)
    {
        ...
        Err(error) => {
            metadata_issues.push(format!(
                "metadata `{key}` must be a JSON object keyed by trusted host hook id: {error}"
            ));
            return Vec::new();
        }
    };
    ...
    let mut specs = Vec::new();
    for (hook, spec_value) in parsed_specs {
        let Some(spec_object) = spec_value.as_object() else {
            metadata_issues.push(format!(
                "trusted host hook spec `{hook}` in `{key}` must be a JSON object"
            ));
            continue;
        };
        ...
        let sample_payload_json = spec_object
            ...
            .unwrap_or_else(|error| {
                metadata_issues.push(format!(
                    "trusted host hook spec `{hook}` in `{key}` has a non-serializable sample_payload: {error}"
                ));
                None
            });
...
fn normalized_event_specs_with_issue(
    metadata: &BTreeMap<String, String>,
    key: &str,
    metadata_issues: &mut Vec<String>,
) -> Vec<PluginNativeExtensionEventSpec> {
    let Some(raw_value) = metadata.get(key) else {
        return Vec::new();
    };
    if raw_value.trim().is_empty() {
    ...
    let parsed_specs = match serde_json::from_str::<BTreeMap<String, serde_json::Value>>(raw_value)
    {
        ...
        Err(error) => {
            metadata_issues.push(format!(
                "metadata `{key}` must be a JSON object keyed by native extension event id: {error}"
            ));
            return Vec::new();
        }
    };
    ...
    let mut specs = Vec::new();
    for (event, spec_value) in parsed_specs {
        let Some(spec_object) = spec_value.as_object() else {
            metadata_issues.push(format!(
                "native extension event spec `{event}` in `{key}` must be a JSON object"
            ));
            continue;
        };
        ...
        let sample_payload_json = spec_object
            ...
            .unwrap_or_else(|error| {
                metadata_issues.push(format!(
                    "native extension event spec `{event}` in `{key}` has a non-serializable sample_payload: {error}"
                ));
                None
            });
...
mod tests {
    ...
    #[test]
    fn translator_derives_channel_bridge_contract_from_manifest_conventions() {
    ...
    #[test]
    fn translator_marks_declared_channel_bridge_contract_incomplete_when_required_fields_are_missing()
     {
        let descriptor = channel_bridge_descriptor(BTreeMap::new());
        ...
        assert!(!channel_bridge.readiness.ready);
        assert_eq!(
            channel_bridge.readiness.missing_fields,
            vec![
    ...
    #[test]
    fn activation_plan_blocks_declared_channel_bridge_when_manifest_contract_fields_are_missing() {
        let descriptor = channel_bridge_descriptor(BTreeMap::new());
        let translator = PluginTranslator::new();
        let translation = translator.translate_scan_report(&PluginScanReport {
            scanned_files: 1,
        ...
        let setup_readiness_context = PluginSetupReadinessContext {
            ...
            verified_config_keys: BTreeSet::from([
                "weixin.enabled".to_owned(),
                "weixin.bridge_url".to_owned(),
                "weixin.bridge_access_token".to_owned(),
            ]),
        ...
        assert!(
            plan.candidates[0]
                .reason
                .contains("metadata.transport_family"),
            "reason should surface missing transport family"
        );
        assert!(
            plan.candidates[0]
                .reason
...
"#;
    ...
    fn plugin_native_extension_declarations_parse_and_validate_tui_surface() {
        let metadata = BTreeMap::from([
            ...
            (
                ...
                r#"{"turn_start":{"label":"Turn Start","summary":"Observe the start of a turn","sample_payload":{"turn_id":"demo-turn"},"operator_hint":"Dispatches automatically before turn execution."}}"#.to_owned(),
            ),
            (
        ...
        assert_eq!(
            declarations.host_hook_specs[0].label.as_deref(),
            Some("Turn Start")
        );
    ...
    #[test]
    fn plugin_native_extension_declarations_flag_undeclared_host_hook_spec() {
        let metadata = BTreeMap::from([
            (
                "loong_extension_family".to_owned(),
                TRUSTED_HOST_EXTENSION_FAMILY.to_owned(),
            ...
            (
                "loong_extension_host_hook_specs_json".to_owned(),
                r#"{"turn_end":{"label":"Turn End"}}"#.to_owned(),
            ),
        ]);
    ...
    #[test]
    fn plugin_native_extension_declarations_flag_undeclared_event_spec() {
        let metadata = BTreeMap::from([
            (
                "loong_extension_events_json".to_owned(),
                "[\"session_start\"]".to_owned(),
            ...
            (
                "loong_extension_event_specs_json".to_owned(),
                r#"{"turn_start":{"label":"Turn Start Event"}}"#.to_owned(),
            ),
        ]);
