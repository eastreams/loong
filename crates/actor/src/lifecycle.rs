...
pub enum Shutdown {
    ...
    Stop,
    /// Dispatches eligible messages from the queue fixed when Drain committed.
    /// It finishes every resulting reply.
    ///
    /// Pending actor initialization finishes first.
...
/// A typed, non-owning handle for a registered child actor.
///
