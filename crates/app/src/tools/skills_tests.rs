...
impl ScopedHomeFixture {
    ...
    fn _set_env(&mut self, key: &'static str, value: impl AsRef<std::ffi::OsStr>) {
...
fn resolve_download_plan_for_clawhub_falls_back_to_mirror_route() {
    ...
    let plan =
        resolve_external_skill_download_plan("https://clawhub.ai/skills/hybrid-deep-search", &http)
...
fn list_marks_non_executable_required_bin_as_ineligible() {
    with_managed_runtime_test(|| {
        ...
        fs::set_permissions(&fake_bin, permissions).expect("mark fake binary as non-executable");

        home._set_env("PATH", &bin_dir);
        write_file(
            &home.path,
    ...
    with_managed_runtime_test(|| {
        ...
        assert!(
            list_outcome.payload["skills"]
                ...
                .expect("skills should be an array")
                .iter()
                .any(|skill| { skill["skill_id"] == "portable-skill" && skill["scope"] == "user" }),
...
fn provider_surface_hides_model_hidden_skills_and_snapshot_auto_exposure() {
    with_managed_runtime_test(|| {
        ...
        let lines =
            installed_skill_snapshot_lines_with_config(&config).expect("snapshot should succeed");
        assert!(
            lines.is_empty(),
...
fn replace_failed_install_preserves_previous_managed_skill() {
    with_managed_runtime_test(|| {
        ...
        assert!(
            error.contains("cannot contain symlinks") || error.contains("does not allow symlinks"),
