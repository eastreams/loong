...
pub struct ChannelOutboundHttpPolicy {
...
pub fn response_body_detail(body: &str) -> String {
...
pub fn redact_endpoint_status_url(raw_url: &str) -> Option<String> {
    let trimmed_url = raw_url.trim();
...
pub fn redact_generic_webhook_status_url(raw_url: &str) -> Option<String> {
    let trimmed_url = raw_url.trim();
    let parsed_url = reqwest::Url::parse(trimmed_url).ok()?;
    let mut redacted_url = parsed_url;
    let _ = redacted_url.set_username("");
