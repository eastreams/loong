//! Input repair for the unmigrated `ToolCoreRequest` fallback.
//!
//! Registered tools preserve `ToolInputError` inside `ToolInputFailure` and do
//! not consult this catalog or recover structure from an error string.
...
pub(crate) fn detect_repairable_tool_request_issue(
    ...
    // This guard is the architectural boundary: a newly registered tool must
    // never regain this legacy reason-parser path through catalog metadata.
    if descriptor.owner != tools::ToolOwner::LegacyCore {
        return None;
    }
...
fn effective_payload_for_descriptor(
    ...
    if request_tool_name == "tool.invoke" {
        let peeked_request =
            tools::peek_tool_invoke_request(request.tool_name.as_str(), &request.payload)?;
...
#[cfg(test)]
mod tests {
    use super::render_tool_input_repair_guidance_from_reason;
