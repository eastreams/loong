...
fn augment_tool_payload_for_kernel(
    ...
    if canonical_tool_name == "tool.search" {
        payload = inject_tool_search_visible_ids_context_trusted(
            payload,
            &session_context.tool_view,
...
fn inject_tool_search_visible_ids_context_trusted(
    payload: serde_json::Value,
    tool_view: &ToolView,
    ...
    let visible_tool_ids = tool_view
        .tool_names()
        .map(str::to_owned)
        .collect::<Vec<_>>();
    let tool_search_context = json!({
        crate::tools::LOONGCLAW_INTERNAL_TOOL_SEARCH_VISIBLE_TOOL_IDS_KEY: visible_tool_ids,
    });
    internal.insert(
