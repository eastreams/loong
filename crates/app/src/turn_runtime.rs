...
use crate::config::{self, LoongConfig};
use crate::conversation::{
    ConversationRuntimeBinding, ConversationSessionAddress, ConversationTurnCoordinator,
};
#[cfg(feature = "memory-sqlite")]
use crate::session::LATEST_SESSION_SELECTOR;
#[cfg(feature = "memory-sqlite")]
use crate::session::latest_resumable_root_session_id;
#[cfg(feature = "memory-sqlite")]
...
pub(crate) fn initialize_cli_turn_runtime_with_loaded_config(
    ...
    if initialize_runtime_environment {
        crate::runtime_env::export_runtime_environment(&config, Some(&resolved_path));
    }

    crate::runtime_env::initialize_runtime_singletons(&config, Some(&resolved_path));
