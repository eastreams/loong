...
fn session_message_event_is_targetable_without_changing_counts() {
    ...
    let event = manager.record_session_message(serde_json::json!({ "session_id": "s-1" }), true);
...
fn pairing_registry_requires_repairing_for_scope_upgrade() {
    ...
    let upgraded_scopes = BTreeSet::from(["operator.read".to_owned(), "operator.acp".to_owned()]);
...
fn pairing_registry_does_not_mutate_memory_when_approval_persistence_fails() {
    ...
    let registry = broken_pairing_registry_with_request("approve-persist-failure", request.clone());
...
fn pairing_registry_does_not_mutate_memory_when_rejection_persistence_fails() {
    ...
    let registry = broken_pairing_registry_with_request("reject-persist-failure", request.clone());
