...
use super::catalog_executor::{
    ModelCatalogRequestRuntime, fetch_available_models_with_policy, fetch_model_catalog_with_policy,
};
use super::policy;
...
pub(super) async fn fetch_model_catalog_with_profiles(
    config: &LoongConfig,
    ...
    let mut last_error = None;
    for profile in &auth_profiles {
        match fetch_model_catalog_with_policy(ModelCatalogRequestRuntime {
            provider: &config.provider,
            headers: &headers,
            request_policy: &request_policy,
            auth_profile: Some(profile),
