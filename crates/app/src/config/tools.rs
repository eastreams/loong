...
"#;
    ...
    fn runtime_plugins_resolved_roots_auto_discover_loong_extension_dirs_when_unconfigured() {
        ...
        let roots = config
            .resolved_roots()
            .into_iter()
            .map(|path| dunce::canonicalize(path).expect("canonical resolved root"))
            .collect::<Vec<_>>();
        let expected_roots = vec![
            dunce::canonicalize(&project_root).expect("canonical project root"),
            dunce::canonicalize(&global_root).expect("canonical global root"),
        ];
    ...
    fn runtime_plugins_validate_allows_enabled_mode_without_explicit_roots() {
        ...
        assert!(
            ...
            "runtime plugin validation should rely on auto-discovery roots when explicit roots are absent: {issues:?}"
        );
    }
