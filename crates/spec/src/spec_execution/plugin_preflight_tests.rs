...
use super::super::plugin_preflight_policy::ResolvedPluginPreflightPolicy;
use super::*;
use crate::spec_runtime::{PluginPreflightPolicyProfile, PluginPreflightRuleProfile};
...
fn rules_for(
    ...
    policy.rules_for(profile).clone()
}

fn resolved_test_policy(
    profile: PluginPreflightPolicyProfile,
    checksum_seed: char,
) -> ResolvedPluginPreflightPolicy {
    ResolvedPluginPreflightPolicy {
        profile,
        source: "bundled:test".to_owned(),
        checksum: "checksum".to_owned(),
        sha256: checksum_seed.to_string().repeat(64),
    }
}
...
fn runtime_activation_profile_blocks_activation_errors() {
    ...
    let rules = rules_for(&policy, PluginPreflightProfile::RuntimeActivation);
