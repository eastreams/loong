...
use crate::error::{AuthorizationIdentityKind, PermissionRequestError};
...
struct TestContext {
    ...
    revoke_on_user_permission: bool,
    post_user_permission_capability_snapshots: Mutex<VecDeque<Capabilities>>,
...
struct CollectingBackend {
    ...
    revoke_capabilities_during_decide: bool,
    writes: AtomicUsize,
    evidence: Mutex<Vec<AuthorizationEvidence>>,
}

impl CollectingBackend {
    fn new(report: PolicyReport) -> Self {
        Self {
...
fn user_permission_report() -> PolicyReport {
    PolicyReport {
        evaluations: Vec::new(),
