...
#[test]
fn ask_cli_responses_empty_content_tool_calls_chain_continues_until_final_text() {
    let fixture = LatestSelectorCliFixture::new("ask-responses-empty-tool-chain-e2e");
    let alpha_contents = "RESPONSES_EMPTY_ALPHA";
    let bravo_contents = "RESPONSES_EMPTY_BRAVO";
    let alpha_path = fixture.root_path().join("alpha.txt");
    let bravo_path = fixture.root_path().join("bravo.txt");
    ...
    let final_reply = "E2E PASS responses empty-content tool-call chain.";
    let provider_responses = vec![
        MockProviderResponse::ok_json(openai_responses_function_call_body(
            "call-responses-alpha",
            "read",
            json!({
                "path": "alpha.txt",
            }),
        ...
        MockProviderResponse::ok_json(openai_responses_function_call_body(
            "call-responses-bravo",
            "read",
            json!({
                "path": "bravo.txt",
            }),
        )),
        MockProviderResponse::ok_json(openai_responses_final_body(final_reply)),
    ];
    let provider_server = ScriptedMockProviderServer::spawn(provider_responses);
    let provider_base_url = provider_server.base_url().to_owned();
    fixture.write_config_with(|config| {
        ...
        config.provider.base_url = provider_base_url;
        config.provider.model = "test-model".to_owned();
        config.provider.wire_api = ProviderWireApi::Responses;
        config.provider.api_key = Some(SecretRef::Inline("test-provider-key".to_owned()));
    });
    ...
    let output = fixture.run_process(
        &[
            "ask",
            "--message",
            "Read alpha.txt, read bravo.txt, then summarize both files.",
        ],
        None,
    );
    let stdout = render_output(&output.stdout);
    ...
    assert!(
        output.status.success(),
        "ask responses empty tool chain should succeed, stdout={stdout:?}, stderr={stderr:?}"
    );
    assert!(
        stdout.contains(final_reply),
        "stdout should contain the terminal assistant answer: {stdout:?}"
    );
    assert!(
        !stdout.contains(alpha_contents),
        "stdout should not stop at the first tool result: {stdout:?}"
    );
    assert_eq!(
        provider_requests.len(),
        3,
        "ask should continue through both Responses function-call turns: {provider_requests:#?}"
    );

    let alpha_followup_request = &provider_requests[1];
    assert_provider_request_contains_text(
        alpha_followup_request,
        alpha_contents,
        "alpha follow-up provider request",
    );

    let bravo_followup_request = &provider_requests[2];
    assert_provider_request_contains_text(
        bravo_followup_request,
        bravo_contents,
        "bravo follow-up provider request",
    );
}

#[test]
fn ask_cli_repeated_tool_search_ping_pong_injects_guard_before_final_reply() {
    let fixture = LatestSelectorCliFixture::new("ask-tool-search-ping-pong-e2e");
    let guarded_reply = "I should stop and ask before continuing.";
    let provider_responses = vec![
        MockProviderResponse::ok_json(openai_chat_tool_call_body(
            "Let me search for alpha first.",
            "call-search-alpha-1",
            "tool_search",
            json!({
                "query": "alpha note",
        ...
        MockProviderResponse::ok_json(openai_chat_final_body(guarded_reply)),
    ];
    let provider_server = ScriptedMockProviderServer::spawn(provider_responses);
    let provider_base_url = provider_server.base_url().to_owned();
    fixture.write_config_with(|config| {
    ...
    let output = fixture.run_process(
        &[
            "ask",
            "--message",
            "Find the right tool without bouncing between alpha and beta searches.",
        ],
        None,
    );
    let stdout = render_output(&output.stdout);
    ...
    assert!(
        output.status.success(),
        "ask ping-pong e2e should succeed with a guarded reply, stdout={stdout:?}, stderr={stderr:?}"
    );
    assert!(
        stdout.contains(guarded_reply),
        "stdout should contain the guarded terminal reply: {stdout:?}"
    );
    assert!(
        !stdout.contains("completed"),
        "stdout should not fabricate a success summary after ping-pong search: {stdout:?}"
    );
    assert_eq!(
        provider_requests.len(),
        5,
        "ask should continue through the ping-pong guard follow-up: {provider_requests:#?}"
    );

    let guard_followup_request = &provider_requests[4];
    assert_provider_request_contains_text(
        guard_followup_request,
        "[tool_loop_warning]",
        "ping-pong guard provider request",
    );
    assert_provider_request_contains_text(
        guard_followup_request,
