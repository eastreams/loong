...
use crate::onboard::import::select_non_interactive_starting_config;
...
fn provider_model_probe_transport_failure_prioritizes_route_guidance() {
    ...
    assert!(
        check
            .detail
            .contains(crate::provider::route_diagnostics::MODEL_CATALOG_TRANSPORT_FAILED_MARKER),
...
fn interactive_entry_screen_omits_static_options_when_selection_widget_handles_choices() {
    ...
    assert!(
        lines
            .iter()
            .any(|line| line == crate::onboard::presentation::entry_choice_section_heading()),
...
fn interactive_starting_point_screen_omits_static_options_when_selection_widget_handles_choices() {
    ...
    assert!(
        lines
            .iter()
            .any(|line| line == crate::onboard::presentation::starting_point_selection_title()),
...
fn preflight_summary_uses_explicit_model_only_guidance_without_reviewed_default() {
    ...
    assert!(
        lines.iter().any(|line| {
            line == crate::onboard::presentation::preflight_explicit_model_only_rerun_hint()
        }),
...
fn preflight_summary_omits_skip_model_probe_rerun_hint_after_probe_is_already_skipped() {
    ...
    assert!(
        lines.iter().all(|line| {
            line.as_str() != crate::onboard::presentation::preflight_probe_rerun_hint()
        }),
