use super::*;
use crate::onboard_cli::review::{build_onboard_review_digest_display_lines, onboard_display_line};
use crate::onboard_cli::shortcut_write_render::render_shortcut_default_choice_footer_line;

pub(super) fn render_onboarding_risk_screen_lines_with_style(
    width: usize,
    color_enabled: bool,
) -> Vec<String> {
    let copy = crate::onboard::presentation::risk_screen_copy();
...
pub(super) fn build_write_confirmation_screen_spec(
    ...
    let status_line =
        crate::onboard::presentation::write_confirmation_status_line(warnings_kept).to_owned();
    ...
    let choices = vec![
        TuiChoiceSpec {
            key: "y".to_owned(),
            label: crate::onboard::presentation::write_confirmation_label().to_owned(),
            detail_lines: vec![
                crate::onboard::presentation::write_confirmation_detail().to_owned(),
            ],
            recommended: false,
        },
        TuiChoiceSpec {
            key: "n".to_owned(),
            label: crate::onboard::presentation::write_confirmation_cancel_label().to_owned(),
            detail_lines: vec![
                crate::onboard::presentation::write_confirmation_cancel_detail().to_owned(),
            ],
            recommended: false,
        },
    ];
    ...
    let default_choice_line = render_default_choice_footer_line(
        "y",
        crate::onboard::presentation::write_confirmation_default_choice_description(),
    );
    ...
    TuiScreenSpec {
        header_style: TuiHeaderStyle::Compact,
        subtitle: None,
        title: Some(crate::onboard::presentation::write_confirmation_title().to_owned()),
