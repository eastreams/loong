...
pub(crate) fn provider_model_probe_failure(
    ...
    let provider_prefix = crate::provider::presentation::active_provider_detail_label(config);
    let is_transport_failure =
        crate::provider::route_diagnostics::is_transport_style_model_probe_failure(error);
...
pub(crate) fn provider_model_probe_failed_detail(detail: &str) -> bool {
    ...
    let has_transport_failure =
        detail.contains(crate::provider::route_diagnostics::MODEL_CATALOG_TRANSPORT_FAILED_MARKER);
...
fn render_transport_failure_detail(provider_prefix: &str, error: &str) -> String {
    let marker = crate::provider::route_diagnostics::MODEL_CATALOG_TRANSPORT_FAILED_MARKER;
    format!(
