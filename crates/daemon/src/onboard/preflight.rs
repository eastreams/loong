...
pub(crate) async fn run_preflight_checks(
    ...
    if skip_model_probe {
        ...
        match mvp::provider::fetch_available_models(config).await {
            ...
            Err(error) => {
                let probe_failure =
                    crate::provider::model_probe_policy::provider_model_probe_failure(
                        config,
                        error.as_str(),
                    );
                let should_collect_route_probe = matches!(
                    &probe_failure.kind,
                    crate::provider::model_probe_policy::ProviderModelProbeFailureKind::TransportFailure
                );
    ...
    let detail = checks
        ...
        .map(|check| {
            ...
            if check.name == "provider model probe"
                && crate::provider::model_probe_policy::provider_model_probe_transport_failure_detail(
                    check.detail.as_str(),
                )
...
pub fn provider_credential_check(config: &mvp::config::LoongConfig) -> OnboardCheck {
    ...
    let status = crate::provider::credentials_guidance::provider_credential_status(
        provider,
        crate::provider::credential_policy::provider_has_locally_available_credentials(provider),
    );

    OnboardCheck {
        name: crate::provider::credentials_guidance::PROVIDER_CREDENTIALS_LABEL,
...
pub(crate) fn render_preflight_summary_screen_lines_with_progress(
    ...
    let spec = crate::onboard::preflight_presentation::build_preflight_summary_screen_spec(
        checks,
        progress_line,
    );
...
pub fn render_preflight_summary_screen_lines(checks: &[OnboardCheck], width: usize) -> Vec<String> {
    let progress_line = crate::onboard::presentation::review_flow_copy(
        crate::onboard::presentation::ReviewFlowKind::Guided,
    )
    .progress_line;
...
fn provider_check_detail_prefix(config: &mvp::config::LoongConfig) -> String {
    crate::provider::presentation::active_provider_detail_label(config)
}

fn onboard_check_from_provider_model_probe_failure(
    probe_failure: crate::provider::model_probe_policy::ProviderModelProbeFailure,
) -> OnboardCheck {
    let level = match probe_failure.level {
        crate::provider::model_probe_policy::ProviderModelProbeFailureLevel::Warn => {
            OnboardCheckLevel::Warn
        }
        crate::provider::model_probe_policy::ProviderModelProbeFailureLevel::Fail => {
    ...
    let non_interactive_warning_policy = match probe_failure.kind {
        ...
        crate::provider::model_probe_policy::ProviderModelProbeFailureKind::ExplicitModel {
            ..
        } => OnboardNonInteractiveWarningPolicy::AcceptedByExplicitModel,
        crate::provider::model_probe_policy::ProviderModelProbeFailureKind::PreferredModels {
            ..
        } => OnboardNonInteractiveWarningPolicy::AcceptedByPreferredModels,
        crate::provider::model_probe_policy::ProviderModelProbeFailureKind::RequiresExplicitModel {
...
pub(crate) fn provider_model_probe_failure_check(
    ...
    let probe_failure =
        crate::provider::model_probe_policy::provider_model_probe_failure(config, error.as_str());
...
fn provider_route_probe_preflight_check(
    ...
    OnboardCheck {
        name: crate::provider::route_diagnostics::PROVIDER_ROUTE_PROBE_CHECK_NAME,
        level,
