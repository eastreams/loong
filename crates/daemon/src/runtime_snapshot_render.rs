...
use crate::{
    ...
    build_channels_cli_json_payload, context_engine_metadata_json, format_capability_names,
    json_string_field, memory_system_metadata_json, memory_system_policy_json, mvp,
    render_string_list, shell_policy_default_str,
};
...
pub fn build_runtime_snapshot_cli_json_payload(snapshot: &RuntimeSnapshotCliState) -> Value {
    json!({
        ...
        "schema": {
            "version": crate::RUNTIME_SNAPSHOT_CLI_JSON_SCHEMA_VERSION,
        ...
        "runtime_plugins": runtime_snapshot_runtime_plugins_json(&snapshot.runtime_plugins),
...
pub fn render_runtime_snapshot_text(snapshot: &RuntimeSnapshotCliState) -> String {
    ...
    for plugin in &snapshot.runtime_plugins.plugins {
        lines.push(format!(
            "  runtime_plugin {} provider={} connector={} bridge={} status={} slot_claims={} conflicting_slot_claims={}",
            plugin.plugin_id,
            plugin.provider_id,
    ...
    lines.push(format!(
        "external_skills inventory_status={} override_active={} enabled={} require_download_approval={} auto_expose_installed={} install_root={} resolved_skills={} shadowed_skills={} blocked_skills={} ineligible_skills={} blocked_ids={} ineligible_ids={} inventory_error={}",
        ...
        snapshot.external_skills.blocked_skill_count,
        snapshot.external_skills.ineligible_skill_count,
        render_string_list(snapshot.external_skills.blocked_skill_ids.iter().map(String::as_str)),
        render_string_list(
            snapshot
                .external_skills
...
fn runtime_snapshot_external_skills_json(snapshot: &RuntimeSnapshotExternalSkillsState) -> Value {
    json!({
        ...
        "blocked_skill_count": snapshot.blocked_skill_count,
        "ineligible_skill_count": snapshot.ineligible_skill_count,
        "blocked_skill_ids": snapshot.blocked_skill_ids,
        "ineligible_skill_ids": snapshot.ineligible_skill_ids,
        "inventory": snapshot.inventory,
    })
}

fn runtime_snapshot_runtime_plugins_json(snapshot: &RuntimeSnapshotRuntimePluginsState) -> Value {
    json!({
