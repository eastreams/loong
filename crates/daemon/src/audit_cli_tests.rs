...
use crate::kernel::{
    AuditEvent, AuditEventKind, AuditSink, Capability, CapabilityToken, ExecutionPlane, PlaneTier,
};
...
fn audit_token_trail_text_and_json_render_lifecycle() {
    ...
    assert!(
        rendered.contains("loaded_events=3 total_matching_events=4 truncated_matching_events=1")
    );
    ...
    assert!(
        rendered.contains("event_kind_counts=AuthorizationDenied=1,TokenIssued=1,TokenRevoked=1")
    );
    assert!(rendered.contains("issued_event_id=evt-1"));
    assert!(
        rendered
            .contains("issued_capability_count=2 issued_capabilities=invoke_tool,network_egress")
    );
    assert!(rendered.contains(
        "authorization_denied_count=1 authorization_denied_reason_counts=missing capability=1"
    ));
    assert!(rendered.contains(
        "revoked_event_id=evt-3 revoked_timestamp_epoch_s=1700010502 revoked_agent_id=agent-revoke"
    ));
        ...
        &[
            ...
            sample_audit_event(
                ...
                AuditEventKind::ToolSearchEvaluated {
                    ...
                    conflicting_requested_tiers: false,
                    filtered_out_candidates: 1,
                    filtered_out_tier_counts: BTreeMap::from([("unverified".to_owned(), 1_usize)]),
                    top_provider_ids: vec!["community-search".to_owned()],
                },
            ),
        ],
...
fn audit_discovery_groups_by_agent() {
    ...
    assert!(
        rendered.contains("group_drill_down[agent]=agent-a command=loong audit recent --config")
    );
    assert!(rendered.contains(
...
fn audit_discovery_group_correlated_summary_command_broadens_to_workload_window() {
    ...
    let command = discovery_group_correlated_summary_command(&execution, 25, Some("agent"), &group)
...
fn audit_summary_text_includes_triage_counts_and_last_seen_fields() {
    ...
    assert!(
        rendered
            .contains("pack_id=- agent_id=- event_id=evt-3 token_id=token-2 kind=- triage_label=-")
    );
...
fn audit_repair_reports_repaired_for_legacy_prefix() {
    ...
    let payload = audit_cli_json(&execution);
    let verify_report =
        crate::kernel::verify_jsonl_audit_journal(&journal_path).expect("verify repaired journal");
