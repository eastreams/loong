...
5. **15-crate DAG, no cycles** — keep dependency direction strictly acyclic across the full workspace: `loong-core -> contracts`; `loong-access -> {contracts, loong-core}`; `loong-runtime -> {contracts, loong-core, kernel}`; `loong-tools -> {contracts, loong-core, kernel}`; `loong-core` stays below `loong-plugin-sdk`; `kernel -> {contracts, loong-access, loong-core, loong-plugin-sdk}`; `protocol` remains an independent foundation crate; `bridge-runtime -> {contracts, kernel, protocol}`; `app -> {contracts, loong-core, kernel, loong-runtime, loong-tools}`; `spec -> {contracts, loong-core, kernel, protocol, bridge-runtime}`; `bench -> {kernel, spec}`; `daemon` stays the delivery layer over the lower crates, including direct `loong-core` access for core policy type ownership. There are currently no tracked dependency-graph deviations. Dependency direction is non-negotiable. See [Layered Kernel Design](layered-kernel-design.md).
