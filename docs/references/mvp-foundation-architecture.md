...
2. `Application Layer` (`app/conversation/*`)
...
3. `Adapter Layer` (`app/channel/*`, `app/provider/*`, `app/tools/*`, `app/memory/*`)
...
- Telegram polling adapter (`app/channel/telegram.rs`)
- Feishu send + webhook adapter (`app/channel/feishu/*`)
...
1. Add `app/channel/<name>.rs` implementing `ChannelAdapter`.
...
2. Implement request path in `app/provider/mod.rs` (and required submodules).
