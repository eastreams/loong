...
2. **Audit events are never silently dropped** — kernel sinks fail closed on write errors instead of silently downgrading. Production app bootstraps default to `FanoutAuditSink` backed by `~/.loongclaw/audit/events.jsonl`, with a derived integrity sidecar plus `loong audit verify` and `loong audit repair` for local operator validation and controlled recovery. `loong audit repair` only rebuilds journals that are missing the entire sidecar and refuses partially missing or mismatched sidecars. `LoongClawKernel::new()` defaults to `InMemoryAuditSink`, and spec/test/demo helpers may intentionally use explicit in-memory audit seams for side-effect-free reporting. `NoopAuditSink` remains reserved for callers that explicitly opt into `new_without_audit(...)` or wire a noop sink themselves.
