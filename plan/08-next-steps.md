...
2. 完成 unified Context 传播并删除 `KernelContext`：
...
5. 把 tool execution audit 从 contracts/kernel 收回 runtime owner：
   ...
   - legacy `PlaneInvoked` 随步骤 4 的 legacy plane 一起删除，不保留 typed/legacy 宽松双断言；
...
- `TODO(deprecate-kernel-context)` -> 步骤 2；
- `TODO(deprecate-no-kernel-live-source)` /
  `TODO(deprecate-provider-live-source-bridge)` -> 步骤 3；
