...
lane, continue to [Gateway And Supervision](/use-loongclaw/gateway-and-supervision).
...
| --- | --- |
| the main team-chat runtime should come first | Recipe 1 for Feishu / Lark |
| the enterprise lane should come first | Recipe 4 for WeCom |
| the simplest reply-loop bot is enough | Recipe 2 for Telegram |
...
| Need | Start with | Why |
| --- | --- | --- |
| enterprise chat app integration | Feishu / Lark | shipped inbound runtime with webhook or websocket mode |
...
`channels` is the quickest way to see how LoongClaw currently classifies each
surface.
...
- Keep Feishu / Lark, WeCom, Telegram, and Matrix in the runtime-backed lane.
- Keep webhook, email, Slack, Discord, Teams, and similar workplace sinks in the outbound-only lane.
- Jump to [Common Setups](/use-loongclaw/common-setups) when the provider and channel should be chosen together.
...
| Volcengine plus WeCom | [WeCom Rollout](/use-loongclaw/wecom-rollout-playbook) | the provider and official WeCom lane stay together |
| BytePlus Coding plus Telegram | [BytePlus Coding Plus Telegram](/use-loongclaw/byteplus-coding-telegram-playbook) | coding route and lightweight bot surface stay explicit |
| local or self-hosted inference plus outbound delivery | [Local Model Plus Outbound Delivery](/use-loongclaw/local-outbound-playbook) | outbound-only delivery stays truthful |
| several runtime-backed channels on one host | [Gateway Rollout](/use-loongclaw/gateway-rollout-playbook) | owner commands and selectors stay in one rollout path |
...
## Recipe 6: Outbound-Only Webhook, Email, Or Workplace Notifications
...
from_address = "loongclaw@example.com"
```
...
- when you need the ownership and inspection model without the recipe framing,
  switch to [Gateway And Supervision](/use-loongclaw/gateway-and-supervision)
...
| --- | --- |
| move to a full provider-plus-channel rollout path | [Common Setups](/use-loongclaw/common-setups) |
| step back to the conceptual surface model | [Channels](/use-loongclaw/channels) |
| inspect the field-level public setup guide | [Channel Setup](/use-loongclaw/channel-setup) |
| inspect owner commands, selectors, and supervision | [Gateway And Supervision](/use-loongclaw/gateway-and-supervision) |
| go fix the provider lane first | [Providers And Models](/use-loongclaw/providers-and-models) and [Provider Recipes](/use-loongclaw/provider-recipes) |
