...
LoongClaw should own longer-lived delivery.

If you still need initial channel setup or smoke tests, start with
[Channel Recipes](/use-loongclaw/channel-recipes) first. This page is about
runtime ownership, supervision, and account selection after the basic lane is
already healthy.
...
| Included in runtime supervision | Not part of runtime supervision |
| --- | --- |
| Feishu / Lark | Webhook |
| Telegram | Email |
| Matrix | Slack, Discord, Teams, Google Chat |
| WeCom | Twitch, Signal, WhatsApp, and similar outbound-only surfaces |
...
- `channels` shows how LoongClaw currently classifies each surface
...
- Continue to [Gateway Rollout](/use-loongclaw/gateway-rollout-playbook) when
  you want the owner contract turned into a rollout sequence.
- Continue to [Common Setups](/use-loongclaw/common-setups) when you want
  provider, channel, and gateway choices combined into one setup path.
- Go back to [Channels](/use-loongclaw/channels) for the surface model and
  runtime-backed versus outbound-only boundary.
- Continue to [Channel Setup](/use-loongclaw/channel-setup) for the practical
  public setup contract.
- Continue to [Channel Recipes](/use-loongclaw/channel-recipes) for smoke tests,
  worked examples, and rollout walkthroughs.
- The field-level public source spec remains in the repository's
  [Channel Setup](https://github.com/loongclaw-ai/loongclaw/blob/dev/docs/product-specs/channel-setup.md)
  markdown.
