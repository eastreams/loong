...
- legacy flat aliases such as `telegram-send`, `line-serve`, or `onebot-send` remain compatibility surfaces where the runtime still accepts them
...
| one WhatsApp business account should own a Cloud API reply loop | configure `whatsapp` and run `loong channels serve whatsapp` | send and serve are both shipped on the verified webhook path |
...
| WhatsApp | Cloud API credentials plus verified webhook material | `loong channels serve whatsapp` |
...
- grouped `channels serve <surface>` flows and their legacy `*-serve` aliases retry transient managed bridge runtime failures locally with bounded backoff before surfacing a hard failure
...
- `loong feishu send` and `loong feishu serve` are the canonical first-class shipped commands; `feishu-send` and `feishu-serve` remain compatibility aliases
...
- use `loong channels serve qqbot` to run the selected managed bridge reply loop under Loong supervision
- use `loong channels serve qqbot --stop` to request cooperative shutdown for the selected managed bridge runtime owner
- Loong automatically asks older duplicate owners to stop when it can identify one clear preferred runtime owner
- use `loong channels serve qqbot --stop-duplicates` as the operator fallback to keep the preferred live runtime owner and cooperatively drain the duplicate owners
