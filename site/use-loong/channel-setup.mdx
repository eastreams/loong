...
The current WhatsApp lane is a Cloud API plus verified webhook runtime surface:
...
- keep this Cloud API credential flow separate from any QR-linked personal-account pairing path so operators can tell whether they need Meta business credentials or a scan-to-link setup
