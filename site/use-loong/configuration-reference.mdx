...
description: Field-level public reference for Loong's TOML config, validation semantics, providers, tools, memory, channels, and Feishu integration.
---

# Configuration Reference

Use this page when you need the field-level public config contract for Loong.
If you want the shared shape first, start with
[Configuration Patterns](/use-loong/configuration-patterns).

`loong validate-config` performs structural and range diagnostics for provider/channels/tools/memory/feishu_integration fields. Some fields (notably parts of `conversation` and most `acp` runtime knobs) are accepted and then normalized at runtime rather than rejected by validate diagnostics.
...
Existing legacy `~/.loongclaw/` homes are still discovered when `~/.loong/`
...
|-------|------|---------|-------------------|-------------|
| `enabled` | boolean | `true` | `true`, `false` | Enable built-in bounded page browsing |
| `max_sessions` | integer | `8` | `1` - `32` | Maximum concurrent browse sessions |
...
| `prompt_pack_id` | string? | `"loong-core-v1"` | Pack ID | Prompt pack ID |
