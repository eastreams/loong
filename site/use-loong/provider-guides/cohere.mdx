---
title: Cohere
description: Configure the Cohere provider profile in Loong.
---

# Cohere

Use this when your team already runs on Cohere and you want that vendor family to stay explicit in Loong config instead of disappearing behind a generic compatibility route.

## At A Glance

| Field | Value |
| --- | --- |
| Built-in kind | `cohere` |
| Provider group | Direct Hosted Providers |
| Protocol family | `openai_chat_completions` |
| Feature family | `openai_compatible` |
| Auth scheme | `bearer` |
| Credential envs | COHERE_API_KEY |
| Aliases | `cohere_compatible` |
| Default base URL | `https://api.cohere.ai/compatibility` |
| Request endpoint | `https://api.cohere.ai/compatibility/v1/chat/completions` |
...
| Default API key env | `COHERE_API_KEY` |
| OAuth env | none |
| Primary request route | `https://api.cohere.ai/compatibility/v1/chat/completions` |
| Primary model-catalog route | `https://api.cohere.com/v1/models` |
