...
| expose the gateway to a trusted browser frontend or custom WebUI | `loong gateway run` + [Gateway For WebUI](/use-loong/gateway-webui) | keeps the owner contract and the frontend contract explicit |
...
- Continue to [Gateway For WebUI](/use-loong/gateway-webui) when you want
  a browser frontend, desktop client, or custom service to call the live
  gateway over HTTP.
