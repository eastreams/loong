...
The canonical public docs story now uses grouped shells first: `loong channels serve <surface>` for single-surface runtime loops, `loong gateway ...` for persisted ownership, and `multi-channel-serve` as the legacy compatibility wrapper when operators still want the older flat supervisor name.
