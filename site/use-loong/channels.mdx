...
| WeChat or personal WhatsApp ecosystems should route through an external bridge | [Weixin](/use-loong/channel-guides/weixin), [OneBot](/use-loong/channel-guides/onebot), or [WhatsApp Personal](/use-loong/channel-guides/whatsapp-personal) | plugin-backed surfaces keep bridge ownership explicit instead of pretending a native runtime exists |
| QQ should own a native official gateway runtime | [QQ Bot](/use-loong/channel-guides/qqbot) | Loong owns the runtime directly through `channels serve qqbot` or `gateway run` |
...
| Gateway-supervised service channels | shipped reply-loop runtimes that can join `gateway run` | Feishu / Lark, Telegram, Matrix, QQ Bot, WhatsApp Cloud API, WeCom |
...
| Plugin-backed bridge surfaces | shipped channel contracts whose upstream session lifecycle stays in an external bridge or managed plugin | Weixin, OneBot, WhatsApp Personal |
