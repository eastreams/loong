...
- legacy flat aliases such as `telegram-send`, `matrix-serve`, or `wecom-send` remain compatibility surfaces where the runtime still accepts them
...
| WhatsApp | Cloud API plus verified webhook service | `loong channels send whatsapp`, `loong channels serve whatsapp` |
...
6. Reach for `gateway run` or the legacy `multi-channel-serve` wrapper only when you want multiple gateway-supervised service channels attached to the same CLI host.
