...
Use `runtime list models` after credentials are healthy when you want the current catalog from the active provider. The older `list-models` verb remains accepted as a compatibility alias while the grouped `runtime` shell becomes canonical.
